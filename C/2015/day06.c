...
int day06()
{
    ...
    if (fileContents)
    {
        ...
        while (token != NULL)
        {
            ...
            int result = sscanf(token, "turn %s %d,%d through %d,%d", action, &start_x, &start_y, &end_x, &end_y);
            if (result != 5)
            {
                result = sscanf(token, "%s %d,%d through %d,%d", action, &start_x, &start_y, &end_x, &end_y);
                if (result != 5)
                {
