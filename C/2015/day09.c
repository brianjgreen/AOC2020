...
int get_distance(char *point_a, char *point_b)
{
    ...
    {
        if (((strcmp(point_a, d->point_a) == 0) || (strcmp(point_a, d->point_b) == 0)) &&
            ((strcmp(point_b, d->point_a) == 0) || (strcmp(point_b, d->point_b) == 0)))
        {
            return d->distance;
        }
    }
...
int day09()
{
    ...
    if (fileContents)
    {
        ...
        while (token != NULL)
        {
            ...
            char point_b[16];
            int distance;
            result = sscanf(token, "%s to %s = %d", point_a, point_b, &distance);
            if (result != 3)
            {
