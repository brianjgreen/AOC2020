...
#define MD5_HASH_HEX_LENGTH (MD5_LEN * 2 + 1)
...
int day04()
{
    ...
    if (fileContents)
    {
        ...
        while ((!found_part1) || (!found_part2))
        {
            ...
            if ((!found_part1) && (strncmp(md5_hash_hex, five_zeros, strlen(five_zeros)) == 0))
            {
                found_part1 = true;
                printf("2015 Day 04 Part 1: %d %s \n", i, md5_hash_hex);
            }
            if ((!found_part2) && (strncmp(md5_hash_hex, six_zeros, strlen(six_zeros)) == 0))
            {
                found_part2 = true;
