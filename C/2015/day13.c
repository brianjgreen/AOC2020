...
int get_happiness(char *person_a, char *person_b)
{
    ...
    {
        if (((strcmp(person_a, h->person) == 0) || (strcmp(person_a, h->sitting_next_to) == 0)) &&
            ((strcmp(person_b, h->person) == 0) || (strcmp(person_b, h->sitting_next_to) == 0)))
        {
...
int day13()
{
    ...
    if (fileContents)
    {
        ...
        while (token != NULL)
        {
            ...
            result =
                /*
                 * Frank would lose 97 happiness units by sitting
                 * next to Alice.
                 */
                sscanf(token, "%s would %s %d happiness units by sitting next to %s.", person_a, gain_lose,
                       &happy_units, person_b);
            if (result != 4)
            {
