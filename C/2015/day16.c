...
void add_sue(int sue_num, int children, int cats, int samoyeds, int pomeranians, int akitas, int vizslas, int goldfish,
...
int day16()
{
    ...
    if (fileContents)
    {
        ...
        while (token != NULL)
        {
            ...
            // Sue 1:goldfish:6, trees:9, akitas:0
            result = sscanf(token, "Sue %d: %[^:]: %d, %[^:]: %d, %[^:]: %d", &sue_num, present[0], &p[0], present[1],
                            &p[1], present[2], &p[2]);
            if (result != 7)
            {
            ...
            else
            {
                ...
                add_sue(sue_num, children, cats, samoyeds, pomeranians, akitas, vizslas, goldfish, trees, cars,
                        perfumes);
            }
        ...
        for (sue_t *s = sue_head; s != NULL; s = s->next)
        {
            ...
            else if ((s->samoyeds != -1) && (aunt_sue.samoyeds != s->samoyeds))
            {
                continue;
            }
            else if ((s->pomeranians != -1) && (aunt_sue.pomeranians != s->pomeranians))
            {
                continue;
            }
            ...
            else if ((s->goldfish != -1) && (aunt_sue.goldfish != s->goldfish))
            {
                continue;
            }
            ...
            else if ((s->perfumes != -1) && (aunt_sue.perfumes != s->perfumes))
            {
                continue;
            }
