...
monkues = {
    0: {'items': [79, 98], 'op': 'multi', 'param': 19, 'test': 23, 'true': 2, 'false': 3, 'inspect': 0},
    1: {'items': [54, 65, 75, 74], 'op': 'add', 'param': 6, 'test': 19, 'true': 2, 'false': 0, 'inspect': 0},
    2: {'items': [79, 60, 97], 'op': 'squared', 'param': None, 'test': 13, 'true': 1, 'false': 3, 'inspect': 0},
    3: {'items': [74, ], 'op': 'add', 'param': 3, 'test': 17, 'true': 0, 'false': 1, 'inspect': 0}
}

filename = "test.json"
# with open(filename) as data_file:
# # monkies = json.load(data_file)

with open("test.dat.json", 'w') as data_file:
  json.dump(monkues, data_file, indent=4)
