...
class Aoc12:
    ...
    def run_it(self):
        ...
        for i in self.data:
            ...
            if dir == self.left:
                facing = self.rotate_left[
                    (self.rotate_left.index(facing) + int(dist / 90))
                    % len(self.rotate_left)
                ]
            elif dir == self.right:
                facing = self.rotate_right[
                    (self.rotate_right.index(facing) + int(dist / 90))
                    % len(self.rotate_right)
                ]
