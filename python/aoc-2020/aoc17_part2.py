...
class Aoc17:
    def __init__(self):
        ...
        for row in space:
            ...
            for cube in row:
                if cube == '#':
                    self.old_space.append((w, z, y, x))
                x += 1
            y += 1

    def activate_cubes(self, space, check_sector=True):
        for w, z, y, x in space:
            cube_on = False
            total = 0
            if (w, z, y, x) in self.old_space:
                cube_on = True
                total = -1
                ...
                for zz in range(z-1, z+2):
                    for yy in range(y-1, y+2):
                        for xx in range(x-1, x+2):
                            if check_sector and (ww, zz, yy, xx) not in self.check_space and (ww, zz, yy, xx) not in self.old_space:
                                self.check_space.append((ww, zz, yy, xx))
                            if (ww, zz, yy, xx) in self.old_space:
                                total += 1
            if cube_on:
                if total == 2 or total == 3:
                    self.new_space.append((w, z, y, x))
            else:
                if total == 3:
                    self.new_space.append((w, z, y, x))
